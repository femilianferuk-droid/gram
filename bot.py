...
class CreateTaskStates(StatesGroup):
    ...
    providing_max_executions = State()  # Новое состояние для лимита выполнений
...
def create_task(task_type: str, link: str, reward: int, creator_id: int, max_executions: int, chat_id: str = None) -> str:
    ...
    tasks_data[task_id] = {
        ...
        "max_executions": max_executions,  # Максимальное количество выполнений
        "completed_by": [],
...
def get_available_tasks(task_type: str, user_id: int) -> List[dict]:
    ...
    for task_id, task in tasks_data.items():
        if (task["type"] == task_type and 
            ...
            len(task["completed_by"]) < task["max_executions"]):  # Проверка лимита
    ...
    try:
        ...
        if chat_identifier.startswith("@"):
            ...
            try:
                ...
                logger.error(f"Некорректный chat_id: {chat_identifier}")
                return False
        ...
        logger.error(f"Ошибка проверки подписки для пользователя {user_id} в чате {chat_identifier}: {e}")
        return False
...
async def handle_earn_type(callback: CallbackQuery):
    ...
    task_desc = {
        "channel": f"📢 <b>Канал:</b> Подпишитесь на канал",
        "group": f"👥 <b>Группа:</b> Вступите в группу",
        "post": f"📰 <b>Пост:</b> Просмотрите публикацию"
    }
...
async def handle_task_reward(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Рассчитываем максимальное количество выполнений
...
async def handle_task_max_executions(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.info(f"Создано задание {task_id} пользователем {user_id}, тип: {task_type}, выполнений: {max_executions}")
...
async def handle_admin_stats(callback: CallbackQuery):
    ...
    # Всего выполнений (с учетом лимитов)
